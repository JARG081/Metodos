...
def newton_raphson_method(func_str, x0, tol, max_iter):
    ...
    try:
        ...
        df = sp.lambdify(x, sp.diff(f_expr, x), modules=["numpy"])
    ...
    for i in range(1, max_iter + 1):
        ...
        df_x0 = df(x0)
        if df_x0 == 0:
            messagebox.showerror("Error", "Derivada igual a cero. Método no puede continuar.")
            return
        x1 = x0 - f_x0 / df_x0
        error_norm = abs(x1 - x0)
        error_rel = abs(error_norm / x1) if x1 != 0 else 0
        data.append((i, x0, f_x0, df_x0, x1, error_norm, error_rel))
        if error_norm < tol:
            break
        x0 = x1
...
def calcular():
    try:
        ...
        newton_raphson_method(func_str, x0, tol, max_iter)
    except ValueError:
...
columns = ("Iteración", "x0", "f(x0)", "f'(x0)", "x1", "Error Norm.", "Error Rel.")
